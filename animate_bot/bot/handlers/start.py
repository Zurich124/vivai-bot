...
async def cmd_start(message: Message) -> None:
    ...
    if video_path.exists():
        ...
        await message.answer_video(
            ...
            caption=(
                '<tg-emoji emoji-id="5474525960143385880"></tg-emoji> <b>Fulfill Your Desires</b>\n'
    ...
    await message.answer(
        '<tg-emoji emoji-id="5373159350363764070"></tg-emoji> <b>Welcome! / Willkommen!</b>\n\n'
        '🇬🇧 English\n'
        '🇩🇪 Deutsch',
...
async def select_language(callback: CallbackQuery) -> None:
    ...
    if animation_video_path.exists():
        ...
        await callback.message.answer_video(
            ...
            caption=(
                '<tg-emoji emoji-id="5255861796350224063"></tg-emoji> <b>Viva AI — Photo & Video Generation</b>\n\n'
                '<tg-emoji emoji-id="5330237710655306682"></tg-emoji> <b>Directly in Telegram</b>\n\n'
                'Upload a photo → choose a mode → get result in seconds\n\n'
                '<tg-emoji emoji-id="5370975411033356097"></tg-emoji> Around 1,000 modes for photo and video generation\n'
                '<tg-emoji emoji-id="5334885900356688822"></tg-emoji> Extend finished videos infinitely\n'
                '<tg-emoji emoji-id="5271604874419647061"></tg-emoji> Combine different modes into chains\n'
                '<tg-emoji emoji-id="5963318814958423599"></tg-emoji> Cinematic-quality video\n'
                '<tg-emoji emoji-id="5215338500739573534"></tg-emoji> All results are delivered only to you\n'
                '<tg-emoji emoji-id="5388632425314140043"></tg-emoji> Generate videos with sound'
            ),
            parse_mode=ParseMode.HTML
        )
